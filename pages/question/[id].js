...
// Find potential paths with getStaticPaths
export async function getStaticPaths(){
    await dbConnect()
    // Finding questions
    const items = await Post.find({parent: null}).exec();
    // Getting each question id
    const paths = items.map(e => ({params: {id: e.id.toString()}}))
    return {
        paths,
        //Allowing the server to build the new questions pages
        fallback: true,
    }
}
...
export async function getStaticProps({ params }){
    await dbConnect()
    const item = await Post.findById(params.id).exec();
    return {
        props: {
